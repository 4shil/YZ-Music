...
/**
 * Camelot Wheel harmonic mapping and key-shift calculation for DJ AutoMix & Harmonic Autoplay.
 *
...
/**
 * Pure harmonic and rhythmic compatibility scoring for track selection.
 */
object HarmonicScorer {

    fun alignTempoOctave(referenceBpm: Double, candidateBpm: Double): Double {
        if (referenceBpm <= 0.0 || candidateBpm <= 0.0) return candidateBpm
        var bpm = candidateBpm
        while (bpm > referenceBpm * 1.414) bpm /= 2.0
        while (bpm < referenceBpm * 0.707) bpm *= 2.0
        return bpm
    }

    fun harmonicScore(leftKey: String, leftKeyConf: Double, rightKey: String, rightKeyConf: Double): Double {
        if (leftKeyConf < 0.4 || rightKeyConf < 0.4 || leftKey.isBlank() || rightKey.isBlank()) {
            return 0.5
        }
        val k1 = CamelotHarmonics.parseKey(leftKey)
        val k2 = CamelotHarmonics.parseKey(rightKey)
        if (k1 == null || k2 == null) return 0.5

        // Same pitch class & mode -> exact match
        if (k1.pitchClass == k2.pitchClass && k1.isMinor == k2.isMinor) return 1.0

        // Relative major/minor (e.g. C major & A minor -> pitch dist 3, diff modes)
        val dist = min((k1.pitchClass - k2.pitchClass + 12) % 12, (k2.pitchClass - k1.pitchClass + 12) % 12)
        if (k1.isMinor != k2.isMinor && dist == 3) return 0.95

        // Circle of fifths (+1 or -1 on Camelot wheel)
        val c1 = CamelotHarmonics.toCamelot(leftKey)
        val c2 = CamelotHarmonics.toCamelot(rightKey)
        if (c1 != null && c2 != null && c1.last() == c2.last()) {
            val num1 = c1.dropLast(1).toIntOrNull() ?: 0
            val num2 = c2.dropLast(1).toIntOrNull() ?: 0
            val diff = (num2 - num1 + 12) % 12
            if (diff == 1 || diff == 11) return 0.90
            if (diff == 2) return 0.80 // Energy jump
        }

        // Parallel major/minor (same tonic root, e.g. C maj & C min)
        if (k1.pitchClass == k2.pitchClass && k1.isMinor != k2.isMinor) return 0.75

        // Within 1 or 2 semitones (shiftable)
        if (k1.isMinor == k2.isMinor && dist in 1..2) return 0.70

        return 0.40
    }

    fun rhythmicScore(bpm1: Double, bpmConf1: Double, bpm2: Double, bpmConf2: Double): Double {
        if (bpm1 <= 0 || bpm2 <= 0) return 0.25
        val aligned = alignTempoOctave(bpm1, bpm2)
        val distance = abs(aligned / bpm1 - 1.0)
        val confidence = min(bpmConf1, bpmConf2).coerceIn(0.0, 1.0)
